      ...
      common_vendor.index.login({
        ...
        success: async (res) => {
          ...
          common_vendor.index.request({
            ...
            fail(err) {
              common_vendor.index.__f__("log", "at pages/API/login/login.vue:174", "获取openid失败：", err);
            }
          });
        },
        fail: (err) => {
          common_vendor.index.__f__("log", "at pages/API/login/login.vue:180", "login fail:", err);
          if (err.code == "30002") {
...
，错误码：${err.code}`
        ...
        common_vendor.index.__f__("error", "at pages/API/login/login.vue:251", res);
      });
    },
    async loginByApple(provider, res) {
      ...
      common_vendor.index.__f__("warn", "at pages/API/login/login.vue:285", "此处使用uni-id处理苹果登录，详情参考: https://uniapp.dcloud.net.cn/uniCloud/uni-id");
      common_vendor.index.request({
        url: "https://97fca9f2-41f6-449f-a35e-3f135d4c3875.bspapp.com/http/user-center",
        method: "POST",
        data: {
        ...
        success: (res2) => {
          common_vendor.index.__f__("log", "at pages/API/login/login.vue:294", "uniId login success", res2);
          if (res2.data.code !== 0) {
            common_vendor.index.showModal({
              showCancel: false,
